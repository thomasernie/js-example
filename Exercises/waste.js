...
const branchName = (branchId) => branches.find(branch => branch.id === branchId);

const mapper = cityDatas.city.map(function(data){
    return {
    id:data.id,
    branchId:data.branchId,
    branch:branchName.name,
    connectedCityId:data.connectedCityId,
    connectedCity:data.name,
    name:data.name
}
});
console.log('conncetedCityBranchMaps =', mapper);


// conncetedCityBranchMaps = [                                 
